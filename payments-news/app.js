(() => {
  ...
  function card(item, index) {
    ...
    return `<article class="story-card ${item.featured && index === 0 ? 'featured' : ''}">
      ...
      <div class="impact"><b>Почему это важно</b>${item.impact}</div>
      <div class="card-footer"><span>${item.date}</span><a href="${item.url}" target="_blank" rel="noopener noreferrer">${item.source} ↗</a></div>
    </article>`;
  }
